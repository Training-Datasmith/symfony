...
final class TranslationPushCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$this->enabledLocales) {
            throw new InvalidArgumentException(\sprintf('You must define "framework.enabled_locales" or "framework.translator.providers.%s.locales" config key in order to work with translation providers.', parse_url((string) $provider, \PHP_URL_SCHEME)));
        }
        ...
        if (!$deleteMissing && $force) {
            ...
            $io->success(\sprintf('All local translations has been sent to "%s" (for "%s" locale(s), and "%s" domain(s)).', parse_url((string) $provider, \PHP_URL_SCHEME), implode(', ', $locales), implode(', ', $domains)));
        ...
        if ($deleteMissing) {
            ...
            $io->success(\sprintf('Missing translations on "%s" has been deleted (for "%s" locale(s), and "%s" domain(s)).', parse_url((string) $provider, \PHP_URL_SCHEME), implode(', ', $locales), implode(', ', $domains)));
        ...
        $io->success(\sprintf('%s local translations has been sent to "%s" (for "%s" locale(s), and "%s" domain(s)).', $force ? 'All' : 'New', parse_url((string) $provider, \PHP_URL_SCHEME), implode(', ', $locales), implode(', ', $domains)));
