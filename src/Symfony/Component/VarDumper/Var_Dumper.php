    ...
    private static function register(): void
    {
        ...
        switch (true) {
            ...
            case 'cli' === $format:
                ...
                break;
            case 'server' === $format:
            case $format && 'tcp' === parse_url((string) $format, \PHP_URL_SCHEME):
                $host = 'server' === $format ? $_SERVER['VAR_DUMPER_SERVER'] ?? '127.0.0.1:9912' : $format;
                $accept = $_SERVER['HTTP_ACCEPT'] ?? (\in_array(\PHP_SAPI, ['cli', 'phpdbg', 'embed'], true) ? 'txt' : 'html');
                $dumper = str_contains((string) $accept, 'html') || str_contains((string) $accept, '*/*') ? new HtmlDumper() : new CliDumper();
        ...
        self::$handler = static function ($var, ?string $label = null) use ($cloner, $dumper): void {
