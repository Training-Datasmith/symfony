...
class Stream_Wrapper
{
    /** @var resource|null */
    public $context;
    private Http_Client_Interface|Response_Interface $client;
     ...
     * @return resource
     */
    public static function create_resource(Response_Interface $response, ?Http_Client_Interface $client = null)
    {
        if ($response instanceof Streamable_Interface) {
            $stack = debug_backtrace(\DEBUG_BACKTRACE_PROVIDE_OBJECT | \DEBUG_BACKTRACE_IGNORE_ARGS, 2);
            if ($response !== ($stack[1]['object'] ?? null)) {
                return $response->to_stream(false);
            }
        }
        ...
        return fopen(strtr(self::class, '\\', '-') . '://' . $response->get_info('url'), 'r', false, stream_context_create(['symfony' => $context]));
    }
     ...
     */
    public function bind_handles(&$handle, &$content): void
    {
        $this->handle =& $handle;
    ...
    public function stream_read(int $count): string|false
    {
        ...
        foreach ($this->client->stream([$this->response], $this->blocking ? $this->timeout : 0) as $chunk) {
            try {
                $this->eof = !$chunk->is_timeout();
                if (!$this->eof && !$this->blocking) {
                    return '';
                }
                $this->eof = $chunk->is_last();
                if ($chunk->is_first()) {
                    $this->response->get_status_code();
                    // ignore 3/4/5xx
                }
                if ('' !== $data = $chunk->get_content()) {
    ...
    public function stream_cast(int $cast_as)
    {
        if (\STREAM_CAST_FOR_SELECT === $cast_as) {
            $this->response->get_headers(false);
