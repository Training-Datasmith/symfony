...
final class WordCount extends Constraint
{
    ...
    public function __construct(
        ...
        if (null !== $min && $min <= 0) {
            throw new ConstraintDefinitionException(\sprintf('The "%s" constraint requires the min word count to be a positive integer if set.', self::class));
        }

        if (null !== $max && $max <= 0) {
            throw new ConstraintDefinitionException(\sprintf('The "%s" constraint requires the max word count to be a positive integer if set.', self::class));
        }

        if (null !== $min && null !== $max && $min > $max) {
            throw new ConstraintDefinitionException(\sprintf('The "%s" constraint requires the min word count to be less than or equal to the max word count.', self::class));
        }
