...
class Form_Util
{
     ...
     */
    public static function is_empty(mixed $data): bool
    {
     ...
     */
    public static function merge_params_and_files(array $params, array $files): array
    {
    ...
    private static function merge(mixed $params, mixed $files): mixed
    {
        ...
        if (\is_array($params) && self::is_file_upload($files)) {
            return $files;
            // if the array is a file upload field, it has the precedence
        }
        if (\is_array($params) && \is_array($files)) {
            // if both are lists and both do not contain arrays, then merge them and return
            if (array_is_list($params) && self::does_not_contain_non_file_upload_array($params) && array_is_list($files) && self::does_not_contain_non_file_upload_array($files)) {
        ...
        if (\is_array($params)) {
            return $params;
            // params has the precedence
        }
        if (self::is_file_upload($files)) {
            return $files;
            // if the array is a file upload field, it has the precedence
        }
        return $params;
    }
    private static function is_file_upload(mixed $value): bool
    {
    ...
    private static function does_not_contain_non_file_upload_array(array $array): bool
    {
        foreach ($array as $value) {
            if (\is_array($value) && !self::is_file_upload($value)) {
                return false;
            }
        }
        return true;
