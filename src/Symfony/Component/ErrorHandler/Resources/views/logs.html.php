...
<?php $channelIsDefined = isset($logs[0]['channel']); ?>
    ...
    foreach ($logs as $log) {
        ...
        <tr class="status-<?= $status; ?>" data-filter-level="<?= strtolower((string) $this->escape($log['priorityName'])); ?>"<?php if ($channelIsDefined) { ?> data-filter-channel="<?= $this->escape($log['channel']); ?>"<?php } ?>>
