...
use Symfony\Component\Expression_Language\Syntax_Error;
/**
 * @author Fabien Potencier <fabien@symfony.com>
 *
 * @internal
 */
class Binary_Node extends Node
{
    ...
    public function compile(Compiler $compiler): void
    {
        ...
        if ('matches' == $operator) {
            if ($this->nodes['right'] instanceof Constant_Node) {
                $this->evaluate_matches($this->nodes['right']->evaluate([], []), '');
            } elseif ($this->nodes['right'] instanceof self && '~' !== $this->nodes['right']->attributes['operator']) {
                throw new Syntax_Error('The regex passed to "matches" must be a string.');
            }
        ...
        if (isset(self::FUNCTIONS[$operator])) {
            $compiler->raw(\sprintf('%s(', self::FUNCTIONS[$operator]))->compile($this->nodes['left'])->raw(', ')->compile($this->nodes['right']);
    ...
    public function evaluate(array $functions, array $values): mixed
    {
        ...
        switch ($operator) {
            ...
            case '%':
                ...
                return $left % $right;
            case 'matches':
                return $this->evaluate_matches($right, $left);
        }
    ...
    private function evaluate_matches(string $regexp, ?string $str): int
    {
        set_error_handler(static fn($t, $m) => throw new Syntax_Error(\sprintf('Regexp "%s" passed to "matches" is not valid', $regexp) . substr($m, 12)));
        try {
