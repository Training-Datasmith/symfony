...
$argv = $_SERVER['argv'] ?? [];
$argc = $_SERVER['argc'] ?? 0;
$getEnvVar = static function (string $name, $default = false) use ($argv) {
    ...
    if (null === $phpunitConfig) {
        ...
        $getPhpUnitConfig = static function (?string $probableConfig) use (&$getPhpUnitConfig) {
            if (!$probableConfig) {
                return null;
            }
        ...
        foreach ($argv as $cliArgumentIndex => $cliArgument) {
            ...
            // short option
            if (str_starts_with((string) $cliArgument, '-c')) {
                if ('-c' === $cliArgument && array_key_exists($cliArgumentIndex + 1, $argv)) {
                    $phpunitConfigFilename = $getPhpUnitConfig($argv[$cliArgumentIndex + 1]);
                } else {
                    $phpunitConfigFilename = $getPhpUnitConfig(substr((string) $cliArgument, 2));
                }
                break;
            }
        }
...
if (!file_exists("$PHPUNIT_DIR/$PHPUNIT_VERSION_DIR/phpunit") || $configurationHash !== @file_get_contents("$PHPUNIT_DIR/.$PHPUNIT_VERSION_DIR.md5")) {
    ...
    $stableVersions = array_filter($info['versions'], static fn (string $v): bool => !preg_match('/-dev$|^dev-/', $v));
...
if (isset($argv[1]) && is_dir($argv[1]) && !file_exists($argv[1].'/phpunit.xml.dist')) {
    ...
    $finder = new RecursiveDirectoryIterator($argv[1], FilesystemIterator::KEY_AS_FILENAME | FilesystemIterator::UNIX_PATHS | \FilesystemIterator::SKIP_DOTS);
    $finder = new RecursiveIteratorIterator($finder);
    $finder->setMaxDepth(getenv('SYMFONY_PHPUNIT_MAX_DEPTH') ?: 3);

    foreach ($finder as $file => $fileInfo) {
        if ('phpunit.xml.dist' === $file) {
            $components[] = dirname((string) $fileInfo->getPathname());
        }
    }
    if ($components) {
        array_shift($cmd);
